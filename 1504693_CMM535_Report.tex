...
\begin{alltt}
\hlcom{#Class Distribution}
\hlkwd{summary}\hlstd{(mushroom}\hlopt{$}\hlstd{Edible)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
training and 40 percent testing split as a starting point/baseline. This is to prevent overfitting from occurring.

\begin{knitrout}
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Divide the dataset into 60% training and 40% testing, to prevent overfitting from ocurring}
\hlstd{inTrain} \hlkwb{<-} \hlkwd{createDataPartition}\hlstd{(}\hlkwc{y}\hlstd{=mushroom}\hlopt{$}\hlstd{Edible,} \hlkwc{p}\hlstd{=}\hlnum{0.6}\hlstd{,} \hlkwc{list}\hlstd{=}\hlnum{FALSE}\hlstd{)}
...
\hlkwd{bwplot}\hlstd{(rs,} \hlkwc{layout} \hlstd{=} \hlkwd{c}\hlstd{(}\hlnum{4}\hlstd{,} \hlnum{1}\hlstd{))}
\end{alltt}
\end{kframe}
\end{knitrout}
...
## 3         0.5        0           0.125        0.0 0.0000000 Edible_c1
...
## No pre-processing
## Resampling: Cross-Validated (10 fold) 
## Summary of sample sizes: 4387, 4389, 4388, 4388, 4389, 4388, ... 
## Resampling results across tuning parameters:
## 
##   mtry  Accuracy  Kappa
##    2    1         1    
...
## Prevalence                     0.1653           0.3525              0.2654
## Detection Rate                 0.1653           0.3525              0.2654
## Detection Prevalence           0.1653           0.3525              0.2654
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Split the actual results}
\hlstd{actual} \hlkwb{<-} \hlstd{testingCluster}\hlopt{$}\hlstd{cluster}
\hlkwd{str}\hlstd{(actual)}

\hlcom{#Split the predicted Results}
\hlstd{pred} \hlkwb{<-} \hlstd{predictRFCluster}
\hlkwd{str}\hlstd{(pred)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{verbatim}
##  chr [1:3248] "Edible_c1" "Edible_c2" "Edible_c1" "Edible_c2" ...
##  Factor w/ 4 levels "Edible_c1","Edible_c2",..: 1 2 1 2 2 2 2 2 2 2 ...
\end{verbatim}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Combine the actual and predicted results into a dataframe.}
\hlstd{cols} \hlkwb{=} \hlkwd{data.frame}\hlstd{(}\hlstr{"Actual"} \hlstd{= actual,} \hlstr{"Predicted"} \hlstd{= pred)}

\hlcom{#Convert the both the actual and predict results to characters}
\hlstd{cols}\hlopt{$}\hlstd{Actual} \hlkwb{<-} \hlkwd{as.character}\hlstd{(cols}\hlopt{$}\hlstd{Actual)}
\hlstd{cols}\hlopt{$}\hlstd{Predicted} \hlkwb{<-} \hlkwd{as.character}\hlstd{(cols}\hlopt{$}\hlstd{Predicted)}
\end{alltt}
\end{kframe}
\end{knitrout}
...
\definecolor{shadecolor}{rgb}{0.969, 0.969, 0.969}\color{fgcolor}\begin{kframe}
\begin{alltt}
\hlcom{#Loop for all the rows in cols dataframe}
\hlkwa{for}\hlstd{(row} \hlkwa{in} \hlnum{1}\hlopt{:}\hlkwd{nrow}\hlstd{(cols))\{}

  \hlcom{#split actual/pred again within for loop}
  \hlstd{actualRow} \hlkwb{<-} \hlstd{cols}\hlopt{$}\hlstd{Actual[row]}
  \hlstd{predRow} \hlkwb{<-} \hlstd{cols}\hlopt{$}\hlstd{Predicted[row]}

  \hlcom{#Remove the last three characters in the actual/predicted results }
  \hlcom{# then check if they are the same}
  \hlstd{results[row]} \hlkwb{<-} \hlkwd{substr}\hlstd{(actualRow,} \hlnum{0}\hlstd{,}\hlkwd{nchar}\hlstd{(actualRow)}\hlopt{-}\hlnum{3}\hlstd{)}\hlopt{==} \hlkwd{substr}\hlstd{(predRow,}\hlnum{0}\hlstd{,} \hlkwd{nchar}\hlstd{(predRow)}\hlopt{-}\hlnum{3}\hlstd{)}

  \hlcom{#If results are the same(TRUE) then set 'Yes' otherwise then 'No'}
  \hlkwa{if}\hlstd{(results[row]} \hlopt{==} \hlnum{TRUE}\hlstd{)\{}
    \hlstd{results[row]} \hlkwb{<-} \hlstr{'Yes'}
  \hlstd{\}} \hlkwa{else}\hlstd{\{}
    \hlstd{results[row]} \hlkwb{<-} \hlstr{'No'}
  \hlstd{\}}
\hlstd{\}}
\end{alltt}
\end{kframe}
